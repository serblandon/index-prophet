...
@Component({
  ...
  styleUrl: './rsi-indicator.component.scss'
})
export class RsiIndicatorComponent implements OnInit{
      ...
      this.route.paramMap
        .pipe(
          switchMap((params: ParamMap) => {
            ...
            return this.technicalIndicatorService.getRsiValues(this.ticker);
          })
        )
        .subscribe({
          next: (data: IRsiData[]) => {
            ...
            this.totalDataChart = {
              ...
              datasets: [
                {
                  type: 'line',
                  data: data.map((entity) => entity.rsi),
                  fill: false,
                  pointStyle: false,
                  pointRadius: 3,
                  tension: 0.1
  ...
  exportDataAsCSV() {
    ...
    this.toastr.success(`${this.ticker.toUpperCase()}-CSV-RSI has been successfully downloaded.`, 'Success');
  }
  ...
  exportChartAsPDF() {
    if (this.chartElement) {
      ...
      html2canvas(chartElement).then(canvas => {
        ...
        pdf.save(`${this.ticker.toUpperCase()}-chart-RSI.pdf`);

        this.toastr.success(`${this.ticker.toUpperCase()}-RSI has been successfully exported as PDF.`, 'Success');
      }).catch(error => {
